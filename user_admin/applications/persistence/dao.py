...
class ModelDao():
    ...
    def get_users_of_postgresql(self, target_db) -> str:
        statement = '''SELECT usename, usecreatedb, usesuper
                    FROM pg_catalog.pg_user;'''
        with db.engine.begin() as conn:
            response = conn.exec_driver_sql(statement).all()
        return response
